...
def importDataInfo(path, data_index):
    columns = ['Center','Steering', 'Throttle']
    noOfFolders = len(os.listdir(path))//2
    data = pd.DataFrame()
    for x in range(data_index[0],data_index[1]+1):
...
def balanceData(data,display=True):
    ...
    #ori
    #samplesPerBin =  300
    samplesPerBin = sum((data['Steering']  <= 1)&(data['Steering']>=1))
...
def loadData(path, data):
  ...
  # print("data_ar shape", data.shape)
  # print("data_ar[1,:]", data_ar[1,:])
